...
系统用飞书多维表格作为岗位 / 简历 / 面经的数据中枢。打开 [多维表格模板链接](https://ucncdzmaddi9.feishu.cn/base/VfEwbBBNhaqzEBsK18oc665Qnkh) → 右上角「...」→「**保存副本**」到你的云空间，即可获得全套数据表（57 字段岗位汇总表、简历库、面经库与视图配置，含 3 条示例数据），无需手建任何字段。

> 飞书应用创建、授权、连通性排查等完整步骤见 [docs/feishu-setup.md](docs/feishu-setup.md)。
