...
export function Job51AgentReport() {
  const {
    localData,
    setLocalData,
    saving,
    ...
    return (
      <div className="bg-white border border-[#e8e8e8] rounded-lg p-5 mb-4">
        ...
        <div className="text-xs text-gray-500 mb-1.5">
          将把主简历中的 {report.fields.length} 个字段应用到 51job 本地数据；主简历未提及的字段保留官网原值。应用后点「保存快照」即可作为回写数据源
        </div>
        ...
        <div className="mt-3">
          {!applyConfirmOpen ? (
            ...
            >
              ...
              {applying ? "应用中..." : `应用映射到 51job（${report.fields.length} 个字段）`}
            </Button>
          ) : (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200">
              <span className="text-xs text-amber-700">将覆盖 51job 本地 {report.fields.length} 个字段，未提及字段保留原值，确认？</span>
        ...
        <ReportFieldEdits platform="51job" report={report} onSaved={fetchReport} />
            ...
            <ul className="ml-3 mt-1 list-disc space-y-0.5">
              {[...new Set(report.fields.filter(f => f.changed).map(f => f.path.split('.')[0]))].map(modKey => {
                const labelMap: Record<string, string> = {
                  ...
                  intentions: "求职意向", works: "工作经历",
                  projects: "项目经历", educations: "教育经历",
                  language: "语言能力", skills: "专业技能", certifications: "资格证书"
                }
