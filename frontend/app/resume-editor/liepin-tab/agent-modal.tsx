/**
 * Liepin 自愈 Agent 深度排查弹窗（机械搬迁自 index.tsx 主布局，行为零变化）
 */
"use client"

import { useLiepinCtx } from "./context"

export function LiepinAgentModal() {
  const {
    liepinAgentModalOpen, setLiepinAgentModalOpen, liepinAgentReport, liepinAgentApplying, handleLiepinAgentApplyHeal,
  } = useLiepinCtx()

  if (!(liepinAgentModalOpen && liepinAgentReport)) return null
  ...
  return (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl border border-orange-100 w-full max-w-xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
              {/* Agent Modal Header */}
              <div className="px-6 py-4 bg-gradient-to-r from-[#FF6B00] via-orange-600 to-rose-600 text-white flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-orange-100/90 mt-0.5">
                      9226 端口真机取证与猎聘专属 6 大根因知识库
                    </div>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setLiepinAgentModalOpen(false)}
              ...
              <div className="p-6 space-y-4 text-xs">
                {/* 诊断结论卡片 */}
                <div className="p-4 rounded-xl bg-gradient-to-br from-orange-50/80 to-rose-50/50 border border-orange-200/80 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-gray-900 text-sm flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-[#FF6B00] animate-pulse" />
                      排查目标：【{liepinAgentReport.module_label || liepinAgentReport.module}】
                    </span>
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-orange-100 text-[#FF6B00] border border-[#FF6B00]/30">
                      置信度 {Math.round((liepinAgentReport.confidence || 0.95) * 100)}%
                    </span>
                  </div>
                  ...
                  <div className="pt-2 border-t border-orange-100 space-y-1.5">
                    <div className="font-semibold text-amber-950 flex items-center gap-1.5">
                      ...
                      <span className="font-mono text-orange-700 bg-orange-100/80 px-1.5 py-0.5 rounded text-[11px]">
                        {liepinAgentReport.rule_name || liepinAgentReport.rule_code}
                      </span>
                    </div>
                    <p className="text-[12px] text-gray-700 leading-relaxed">
                      {liepinAgentReport.root_cause}
                    </p>
                  </div>
                </div>

                {/* 自愈处方与行动卡片 */}
                {liepinAgentReport.recipe && (
                  <div className="p-4 rounded-xl bg-emerald-50/60 border border-emerald-100 space-y-2">
                    <div className="font-bold text-emerald-900 text-xs flex items-center gap-1.5">
                      ...
                      <span className="text-emerald-700">{liepinAgentReport.recipe.title}</span>
                    </div>
                    {liepinAgentReport.recipe.details && (
                ...
                {liepinAgentReport.evidence && (
                  <div className="p-3 rounded-lg bg-gray-50 border border-gray-200/80 space-y-1">
                    ...
                    <pre className="text-[10px] text-gray-600 font-mono overflow-x-auto whitespace-pre-wrap">
                      {JSON.stringify(liepinAgentReport.evidence, null, 2)}
                    </pre>
                  </div>
                )}
              </div>
              ...
              <div className="px-6 py-3.5 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={handleLiepinAgentApplyHeal}
                    disabled={liepinAgentApplying}
                    className="px-4 py-1.5 rounded-lg bg-gradient-to-r from-[#FF6B00] to-rose-600 hover:from-[#e55f00] hover:to-rose-700 text-white font-semibold text-xs shadow-sm hover:shadow transition-all disabled:opacity-50 cursor-pointer flex items-center gap-1.5"
                  >
                    {liepinAgentApplying ? "正在应用处方..." : "⚡ 一键应用处方并自动重新回传"}
                  </button>
                </div>
              </div>
            </div>
