...
/**
 * 猎聘年月选择器（照搬 c.liepin.com 出生日期/参加工作时间选择逻辑）
 * 第一步：选年份（每页12年，可翻页）
 * 第二步：选月份（12个月）
 *
 * value 格式: "YYYY年MM月"（如 "1996年11月"），与猎聘存储格式一致
 */

const YEARS_PER_PAGE = 12
const MONTHS = ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"]

interface LiepinYearMonthPickerProps {
  value: string // "YYYY年MM月" 或 ""
...
function parseValue(value: string): { year: string; month: string } {
  ...
  const m = value.match(/(\d{4})年(\d{1,2})月/)
  if (!m) return { year: "", month: "" }
  return { year: m[1], month: m[2].padStart(2, "0") }
}

export function LiepinYearMonthPicker({ value, onChange, placeholder = "请选择", yearRange, disabled }: LiepinYearMonthPickerProps) {
  ...
  const handleMonthClick = (m: number) => {
    ...
    onChange(`${tempYear}年${monthStr}月`)
    setOpen(false)
    setStep("year")
  }
  ...
  return (
    <div className="relative" ref={containerRef}>
      <input
        ...
        className={cn(
          "w-full h-[40px] rounded-md border bg-background px-3 text-sm transition-colors focus:outline-none",
          disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:border-[#FF6B00]",
