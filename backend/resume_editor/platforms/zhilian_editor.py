...
# 智联招聘简历编辑页面
ZHILIAN_RESUME_URL = "https://rd.zhaopin.com/resume"
...
def create_browser():
    ...
    profile_dir = get_platform_profile("zhilian")
...
def scrape_zhilian_resume_fields() -> dict:
    """
    扒取智联招聘在线简历的所有字段
    ...
    return resume_data, page


def save_zhilian_fields(data: dict, output_path: str = None):
    """保存字段到 JSON 文件"""
    if output_path is None:
        output_path = os.path.join(_RESUME_EDITOR_DIR, 'data', 'zhilian_fields.json')
...
def main():
    ...
    try:
        # 采集字段
        fields, page = scrape_zhilian_resume_fields()

        if fields:
            # 保存到文件
            save_zhilian_fields(fields)
