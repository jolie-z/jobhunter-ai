...
# 添加51job_scraper目录到路径
JOB51_SCRAPER_DIR = os.path.join(_BACKEND_DIR, '51job_scraper')
if JOB51_SCRAPER_DIR not in sys.path:
    sys.path.insert(0, JOB51_SCRAPER_DIR)
...
def scrape_51job_resume_fields() -> dict:
    ...
    print("\n🔄 正在导航到51job简历页面...")
    page.goto(JOB51_RESUME_URL, wait_until="networkidle")
    time.sleep(5)
    ...
    if not name_match:
        name_match = re.search(r'id="name"[^>]*>([^<]+)', html)
    resume_data['name'] = {
        'label': '姓名',
        'required': True,
        'type': 'text',
    ...
    return resume_data, p, context


def save_51job_fields(data: dict, output_path: str = None):
    """保存字段到 JSON 文件"""
    if output_path is None:
        output_path = os.path.join(_RESUME_EDITOR_DIR, 'data', '51job_fields.json')
...
def main():
    ...
    try:
        # 采集字段
        fields, p, context = scrape_51job_resume_fields()

        if fields:
            # 保存到文件
            save_51job_fields(fields)
